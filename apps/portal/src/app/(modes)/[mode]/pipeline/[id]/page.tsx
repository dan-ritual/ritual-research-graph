...
export default function OpportunityDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [opportunityId, supabase, router, fetchLinkedEntities, fetchOwners, modeId]);
  ...
  return (
    <>
      ...
      <main className="p-6 max-w-3xl mx-auto">
        <Link
          href={`/${modeId}/pipeline?workflow=${workflow?.slug || "bd"}`}
        ...
        <div className="grid gap-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label className="font-mono text-xs uppercase tracking-[0.08em]">Name</Label>
              ...
              <div className="space-y-2">
                ...
                {currentStage?.description && (
                  <p className="font-serif text-xs text-[rgba(0,0,0,0.45)] italic mt-2">{currentStage.description}</p>
                )}
              </div>

              <div className="space-y-2">
                <Label className="font-mono text-xs uppercase tracking-[0.08em]">Thesis</Label>
                <Textarea value={thesis} onChange={(e) => setThesis(e.target.value)} placeholder="Core value proposition" rows={3} />
              </div>

              <div className="space-y-2">
                <Label className="font-mono text-xs uppercase tracking-[0.08em]">Angle</Label>
                <Textarea value={angle} onChange={(e) => setAngle(e.target.value)} placeholder="Outreach hook or timing rationale" rows={2} />
              </div>

              <div className="space-y-2">
                <Label className="font-mono text-xs uppercase tracking-[0.08em]">Notes</Label>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-[rgba(0,0,0,0.08)]">
                <Link href={`/${modeId}/pipeline?workflow=${workflow?.slug || "bd"}`}>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-mono text-sm uppercase tracking-[0.12em]">Linked Entities</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <LinkedEntityList opportunityId={opportunityId} entities={linkedEntities} onEntityRemoved={fetchLinkedEntities} modePrefix={`/${modeId}`} />
              <div className="pt-4 border-t border-dotted border-[color-mix(in_srgb,var(--mode-accent)_30%,transparent)]">
                <EntityLinker opportunityId={opportunityId} onEntityLinked={fetchLinkedEntities} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="font-mono text-sm uppercase tracking-[0.12em]">Owners</CardTitle>
              <OwnerSelector opportunityId={opportunityId} currentOwnerIds={owners.map((o) => o.id)} onOwnerAdded={fetchOwners} />
            </CardHeader>
            <CardContent>
              <OwnerList opportunityId={opportunityId} owners={owners} onOwnerRemoved={fetchOwners} />
            </CardContent>
          </Card>

          <Card>
            <CardHeader>
              <CardTitle className="font-mono text-sm uppercase tracking-[0.12em]">AI Tools</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex gap-3">
                <Button variant="outline" onClick={handleGenerateStrategy} disabled={generatingStrategy} className="flex-1">
                  {generatingStrategy ? "GENERATING..." : "GENERATE STRATEGY"}
                </Button>
                <Button variant="outline" onClick={handleGenerateEmail} disabled={generatingEmail} className="flex-1">
              ...
              {opportunity?.strategy && (
                <div className="pt-4 border-t border-dotted border-[color-mix(in_srgb,var(--mode-accent)_30%,transparent)]">
                  <button onClick={() => setShowStrategy(!showStrategy)} className="flex items-center justify-between w-full text-left">
                    <span className="font-mono text-xs uppercase tracking-[0.08em] text-[var(--mode-accent)]">Strategy Document</span>
                    <span className="font-mono text-xs">{showStrategy ? "▼" : "▶"}</span>
                  </button>
                  {showStrategy && (
                    <div className="mt-3 p-4 bg-[#FBFBFB] border border-[rgba(0,0,0,0.08)]">
                      <div className="font-serif text-sm whitespace-pre-wrap prose prose-sm max-w-none">{opportunity.strategy}</div>
                    </div>
                  )}
                </div>
              )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-mono text-sm uppercase tracking-[0.12em]">Activity</CardTitle>
            </CardHeader>
                ...
                {activity.length > 0 ? (
                  activity.map((item) => (
                    <div key={item.id} className="px-6 py-3">
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-xs uppercase tracking-[0.05em] text-[var(--mode-accent)]">{item.action.replace(/_/g, " ")}</span>
                        <span className="font-mono text-[10px] text-[rgba(0,0,0,0.45)]">{new Date(item.created_at).toLocaleString()}</span>
                      </div>
                      {item.details && <p className="font-serif text-xs text-[rgba(0,0,0,0.45)] mt-1">{formatActivityDetails(item.action, item.details)}</p>}
                    </div>
                  ))
                ) : (
                  <div className="p-6 text-center">
                    <p className="font-mono text-[10px] text-[rgba(0,0,0,0.35)] uppercase tracking-[0.05em]">No activity yet</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <div className="text-center">
            <p className="font-mono text-[10px] text-[rgba(0,0,0,0.35)]">
              ID: {opportunity?.id?.slice(0, 8)}... | Created {opportunity?.created_at && new Date(opportunity.created_at).toLocaleDateString()}
            </p>
          </div>
        </div>
      </main>

      <EmailModal isOpen={showEmailModal} onClose={() => setShowEmailModal(false)} emailDraft={opportunity?.email_draft || null} />
    </>
  );
}
