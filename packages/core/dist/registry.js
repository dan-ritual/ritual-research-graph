...
export function saveRegistry(registry) {
    ...
    writeFileSync(join(DATA_DIR, 'entities.json'), JSON.stringify(registry, null, 2));
}
...
export function findEntity(registry, nameOrAlias) {
...
export function addEntity(registry, name, type, metadata = {}) {
...
export function addAppearance(registry, entitySlug, appearance) {
    const entity = registry.entities[entitySlug];
    if (!entity) {
        throw new Error(`Entity not found: ${entitySlug}`);
    }
    // Check for duplicate
    const exists = entity.appearances.some(a => a.micrositeId === appearance.micrositeId && a.section === appearance.section);
    if (!exists) {
        entity.appearances.push(appearance);
        entity.updatedAt = new Date().toISOString();
    }
}
export function updateCoOccurrences(registry, micrositeId, entitySlugs) {
...
export function getTopRelatedEntities(registry, entitySlug, limit = 5) {
    const entity = registry.entities[entitySlug];
    if (!entity)
        return [];
...
export function getEntitiesByOpportunity(registry, opportunitySlug) {
    const slugs = registry.opportunityIndex[opportunitySlug] || [];
    return slugs.map(s => registry.entities[s]).filter(Boolean);
}
export function rebuildOpportunityIndex(registry) {
